...
from auth.dependencies import get_current_user, verify_access_token, require_roles
...
@employee_router.get("/all", response_model=list[EmployeeResponse], dependencies=[Depends(require_roles([EmployeeRoles.HR]))])
async def all_employees(db: AsyncSession = Depends(get_db)):
    employees = await employee_service.get_all(db)
    return employees

@employee_router.post("/", response_model=EmployeeResponse, dependencies=[Depends(require_roles([EmployeeRoles.HR]))])
...
@employee_router.get("/search", response_model=list[EmployeeResponse], dependencies=[Depends(require_roles([EmployeeRoles.HR]))])
async def search_employee(name: str, db: AsyncSession = Depends(get_db)):
    employees = await employee_service.search_by_name(db, name)
    return employees

@employee_router.get("/{id}", response_model=EmployeeResponse, dependencies=[Depends(require_roles([EmployeeRoles.HR]))])
async def get_employee(id: int, db: AsyncSession = Depends(get_db)):
    employee = await employee_service.get_by_id(db, id)
    return employee

@employee_router.patch("/{id}", response_model=EmployeeResponse, dependencies=[Depends(require_roles([EmployeeRoles.HR]))])
...
@employee_router.delete("/{id}", dependencies=[Depends(require_roles([EmployeeRoles.HR]))])
...
@employee_router.get("/{id}/address", response_model=list[EmployeeAddressResponse], dependencies=[Depends(require_roles([EmployeeRoles.HR]))])
...
@employee_router.post("/{id}/address", response_model=EmployeeAddressResponse, dependencies=[Depends(require_roles([EmployeeRoles.HR]))])
...
@employee_router.patch("/{employee_id}/address/{address_id}", response_model=EmployeeAddressResponse, dependencies=[Depends(require_roles([EmployeeRoles.HR]))])
...
@employee_router.delete("/{employee_id}/address/{address_id}", dependencies=[Depends(require_roles([EmployeeRoles.HR]))])
...
@employee_router.get("/{employee_id}/department", response_model=list[DepartmentResponse], dependencies=[Depends(require_roles([EmployeeRoles.HR]))])
