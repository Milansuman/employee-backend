...
from auth.dependencies import verify_access_token, require_role
...
@department_router.get("/all", response_model=list[DepartmentResponse], dependencies=[Depends(require_role(EmployeeRoles.HR))])
async def get_all_departments(db: AsyncSession = Depends(get_db)):
    return await service.get_all_departments(db)

@department_router.get("/search", response_model=list[DepartmentResponse], dependencies=[Depends(require_role(EmployeeRoles.HR))])
...
@department_router.get("/{id}", response_model=DepartmentResponse, dependencies=[Depends(require_role(EmployeeRoles.HR))])
async def get_department(id: int, db: AsyncSession = Depends(get_db)):
    return await service.get_department_by_id(db, id)

@department_router.post("/", response_model=DepartmentResponse, dependencies=[Depends(require_role(EmployeeRoles.HR))])
...
@department_router.patch("/{id}", response_model=DepartmentResponse, dependencies=[Depends(require_role(EmployeeRoles.HR))])
...
@department_router.delete("/{id}", dependencies=[Depends(require_role(EmployeeRoles.HR))])
...
@department_router.get("/{id}/employee", response_model=list[EmployeeResponse], dependencies=[Depends(require_role(EmployeeRoles.HR))])
...
@department_router.put("/{id}/employee", dependencies=[Depends(require_role(EmployeeRoles.HR))])
...
@department_router.delete("/{id}/employee", dependencies=[Depends(require_role(EmployeeRoles.HR))])
